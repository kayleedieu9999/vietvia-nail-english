 ...
 * The goal itself is the learner's own "Mục tiêu mỗi ngày" from Settings.
 */
export default function DailyGoalCard() {
  const progress = useProgress();
  const { dailyGoalMinutes } = useLocalSettings();
  const { minutesToday } = computeStreak(progress.lessons);
  const pct = Math.min(100, Math.round((minutesToday / dailyGoalMinutes) * 100));
      ...
      <div>
        ...
        <div className="mt-2 flex items-center gap-2">
          ...
          <p className="text-lg font-extrabold text-slate-900">
            {minutesToday} / {dailyGoalMinutes} phút
          </p>
        </div>
